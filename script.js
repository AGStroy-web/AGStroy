...
const translations = {
  ...
  ru: {
    ...
    heroTitle:
      "Создаём пространство, <span>в котором хочется жить.</span>",
    ...
    visualLabel:
      "Декоративная иллюстрация строительного интерьера",
    ...
    aboutTitle:
      "Практичный подход.<br><span>Чистый результат.</span>",
    ...
    point1:
      "Акцент на аккуратность и качество исполнения",

    point2:
      "Решения под размеры и особенности объекта",
    ...
    contactsTitle:
      "Расскажите,<br><span>что нужно сделать.</span>",
  ...
  en: {
    ...
    visualLabel:
      "Decorative construction interior illustration",
    ...
    aboutTitle:
      "Practical approach.<br><span>Clean result.</span>",
    ...
    point1:
      "Focus on clean, high-quality execution",

    point2:
      "Solutions tailored to the space and its specifics",
  ...
  hy: {
    ...
    visualLabel:
      "Շինարարական ինտերիերի դեկորատիվ պատկեր",
    ...
    servicesTitle:
      "Ամբողջական վերանորոգում՝ բանալիով",
    ...
    point1:
      "Շեշտը՝ մաքուր և որակյալ կատարման վրա",
...
function applyStaticTranslations() {
  ...
  document
    ...
    .forEach((card, index) => {
      ...
      if (title && tr.serviceTitles?.[index]) {
        title.textContent =
          tr.serviceTitles[index];
      }

      if (text && tr.serviceTexts?.[index]) {
        text.textContent =
          tr.serviceTexts[index];
      }

      if (link) {
        link.textContent = tr.discuss;
      }
  ...
  document
    ...
    .forEach((card, index) => {
      ...
      if (title && tr.processTitles?.[index]) {
        title.textContent =
          tr.processTitles[index];
      }

      if (text && tr.processTexts?.[index]) {
        text.textContent =
          tr.processTexts[index];
      }
...
function renderWorks() {
  ...
  const totalPages =
    Math.ceil(works.length / perPage);
        ...
        return `
          <article class="work-card">
            ...
            <button
              ...
              aria-label="Открыть фотографию ${globalIndex}"
            >

              <img
                src="${work.image}"
                alt="AGStroy — пример работы ${globalIndex}"
                loading="lazy"
                onerror="this.closest('.work-card').style.display='none';"
              />

              <span class="work-card__zoom">
                ↗
              </span>
...
function renderPagination(totalPages) {
  ...
  pagination.querySelectorAll(".pagination__button:not(:disabled)").forEach(button => {
...
pagination.addEventListener(
  ...
  (event) => {
    ...
    if (
      !page ||
      page < 1 ||
      page > Math.ceil(works.length / perPage)
    ) {
      return;
    }
    ...
    // Плавно прокручиваем к портфолио
    document
      .querySelector("#works")
      ?.scrollIntoView({
        behavior: "smooth",
